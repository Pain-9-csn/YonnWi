...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    h2 { font-size: 34px; margin-bottom: 8px; color: var(--dark); font-weight: 800; }
    .subtitle { color: #6b7280; margin-bottom: 24px; line-height: 24px; font-size: 14px; }
    input {
      width: 100%; height: 52px; border: none; outline: none;
      border-radius: 16px; padding: 0 18px; margin-bottom: 14px;
      font-size: 15px; background: var(--soft-bg);
      border: 1px solid transparent; transition: .3s; color: #1f2937;
    }
    input:focus {
    ...
    button {
      ...
      cursor: pointer; transition: .35s ease; margin-top: 8px;
      box-shadow: 0 10px 25px rgba(113,197,93,.28);
    }
    button:hover { transform: translateY(-3px); box-shadow: 0 14px 30px rgba(113,197,93,.35); }
    button:active { transform: scale(.98); }
    .overlay {
      position: absolute; top: 0; left: 50%; width: 50%; height: 100%;
      transition: all .7s cubic-bezier(.77,0,.18,1);
      background: linear-gradient(145deg, #71c55d, #5fb04d); overflow: hidden;
    }
    .overlay::before {
      content: '☪'; position: absolute; top: 40px; right: 50px;
      font-size: 180px; color: rgba(255,255,255,.08); font-weight: bold;
    }
    .overlay::after {
    ...
    .container.active .overlay { transform: translateX(-100%); }
    .overlay-content {
      position: relative; z-index: 2; height: 100%;
      display: flex; flex-direction: column; justify-content: center;
      align-items: center; text-align: center; padding: 50px; color: #fff;
    }
    .logo {
    ...
    .overlay h1 { font-size: 46px; margin-bottom: 18px; font-weight: 800; }
    .overlay p { line-height: 30px; font-size: 16px; max-width: 340px; opacity: .96; margin-bottom: 34px; }
    .switch-btn {
    ...
    .switch-btn:hover { background: #fff; color: #5fb04d; }
    .alert { padding: 12px 16px; border-radius: 14px; margin-bottom: 16px; font-size: 14px; font-weight: 600; }
    .error { background: #fff1f1; color: #dc2626; border: 1px solid #fecaca; }
    .success { background: #f0fdf4; color: #15803d; border: 1px solid #bbf7d0; }

    @media (max-width: 768px) {
      .container { width: 100%; height: auto; }
      .overlay { display: none; }
      .form-container { position: relative; width: 100%; transform: none !important; opacity: 1 !important; padding: 40px 25px; }
